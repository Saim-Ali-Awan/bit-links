...
const Shorten = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        showAlert('Success', 'Short URL created!');
        setUrl('');
        setShorten('');
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (data.success) {
        showAlert('Deleted', 'Link removed successfully');
        ...
        <AlertDialogContent>
          ...
          <div className="flex justify-end gap-2 mt-4 text-black">
            <AlertDialogAction asChild>
              <LiquidButton size="md" variant={'default'} className={'text-black'} onClick={() => setAlertOpen(false)}>OK</LiquidButton>
            </AlertDialogAction>
          </div>
        </AlertDialogContent>
      </AlertDialog>

      <main className="min-h-screen flex flex-col relative items-center pt-28 px-4 pb-20">
        {/* Heading */}
